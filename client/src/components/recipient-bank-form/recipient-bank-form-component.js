...
const RecipientBankForm = () => {
  ...
  const handleChange = useCallback(
    (dataFromChild) => {
      ...
      for (let key in dataFromChild) {
        if (/amount/.test(key)) {
          setValidateInput((prevValid) => ({
            ...prevValid,
            [key]: {
              ...prevValid[key],
              isValid: prevValid[key].validationLogic(dataFromChild[key]),
            },
          }));
        }
        ...
        if (/accountNumber/.test(key)) {
          ...
          if (
            /0223137469/.test(dataFromChild[key]) &&
            /058/.test(recipientData.bank)
          ) {
            timeoutID.current = setTimeout(() => {
              ...
              setValidateInput((prev) => ({
                ...prev,
                accountNumber: { isValid: true, shouldValidate: true },
              }));
            }, 1500);
          }
        }
  ...
  const handleSelect = useCallback((dataFromChild) => {
    setValidateInput((prev) => ({
      ...prev,
      accountNumber: { ...prev.accountNumber, shouldValidate: false },
    }));
  ...
  async function handleSubmit() {
    try {
      const data = await axios.post("/fluttersave/withdraw", {
        fromEmail: "johndoe@anonymous.com",
        amount: "5000",
  ...
  return (
    <Form title="Bank Withdrawal Details" onSubmit={handleSubmit}>
      <InputField
        type="amount"
        placeHolder="Amount"
        value={recipientData.amount}
        onChange={handleChange}
        validateInput={validateInput.amount}
      ...
      <InputField
        ...
        onChange={handleChange}
        validateInput={validateInput.accountNumber}
        setShouldValidate={setShouldValidate}
